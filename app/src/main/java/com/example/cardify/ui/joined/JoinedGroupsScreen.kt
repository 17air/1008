        ...
        if (userId.isBlank()) {
            ...
            val registration = LocalGroupRepository.observeGroups(
                onSuccess = { list ->
                    isLoading = false
                    hasError = false
                    groups = list.filter { LocalGroupRepository.isUserMember(it.id, userId) }
                },
                onError = {
                    isLoading = false
                    hasError = true
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text(text = stringResource(id = R.string.group_actions_joined_groups)) },
                navigationIcon = {
        ...
        when {
            ...
            groups.isEmpty() -> {
                Column(
                    ...
                    Text(
                        text = stringResource(id = R.string.joined_groups_empty),
                        style = MaterialTheme.typography.bodyMedium
                    )
                }
            }
