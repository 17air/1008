            ...
            if (messages.isEmpty()) {
                ...
                LazyColumn(state = listState, modifier = Modifier.fillMaxSize()) {
                    items(messages) { message ->
                        MessageBubble(message = message, isOwnMessage = message.userId == currentUserId)
                    }
                }
            }
        }
    ...
    Column(
        ...
        Surface(
            ...
            Column(modifier = Modifier.padding(12.dp)) {
                Text(text = message.userName.ifEmpty { stringResource(id = R.string.user_guest_placeholder) }, style = MaterialTheme.typography.labelMedium, color = textColor)
